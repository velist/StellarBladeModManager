...
class MainWindow(QMainWindow):
    ...
    def rename_category(self, item):
        ...
        if ok and new_name and new_name != old_name:
            try:
                # 记录是否是默认分类被重命名
                is_default_category = (old_name == self.config.default_category_name)
                ...
                for mod_id, mod_info in mods.items():
                    ...
                    if mod_category == old_name:
                        mod_info['category'] = new_name
                        self.config.update_mod(mod_id, mod_info)
                        updated_mods_count += 1
                        print(f"[调试] rename_category: 更新MOD {mod_id} 的分类从 {old_name} 到 {new_name}")
                
                print(f"[调试] rename_category: 共更新了 {updated_mods_count} 个MOD的分类信息")
    ...
    def rename_subcategory(self, item):
        ...
        if ok and new_name and new_name != old_name:
            try:
                ...
                for mod_id, mod_info in mods.items():
                    mod_category = mod_info.get('category', '')
                    if mod_category == old_full_path:
                        mod_info['category'] = new_full_path
                        self.config.update_mod(mod_id, mod_info)
                        updated_mods_count += 1
                        print(f"[调试] rename_subcategory: 更新MOD {mod_id} 的分类从 {old_full_path} 到 {new_full_path}")
                
                print(f"[调试] rename_subcategory: 共更新了 {updated_mods_count} 个MOD的分类信息")
    ...
    def delete_category(self, item):
        ...
        if name == self.config.default_category_name:
            self.show_message('提示', f'默认分类"{self.config.default_category_name}"无法删除。')
            return
    ...
    def on_item_clicked(self, item):
        ...
        # 如果点击的是分类或子分类，刷新MOD列表
        elif isinstance(data, dict) and data.get('type') in ['category', 'subcategory']:
            print(f"[调试] on_item_clicked: 点击了分类，刷新MOD列表: {data.get('name')}")
            # 强制刷新MOD列表
    ...
    def refresh_mod_list(self, search_text=None, keep_selected=True):
        ...
        print(f"[调试] refresh_mod_list: 分类 {selected_category} 中共有 {mods_in_category} 个MOD")
        print(f"[调试] refresh_mod_list: 已添加 {len(added_mod_ids)} 个MOD到列表")
        
        # 检查是否有异常情况：分类中有MOD但列表为空
        if mods_in_category > 0 and len(added_mod_ids) == 0 and not search_text and self.active_tab == "all":
            print(f"[警告] refresh_mod_list: 检测到异常情况，分类 {selected_category} 中有 {mods_in_category} 个MOD，但列表为空")
            # 可以添加额外处理...
